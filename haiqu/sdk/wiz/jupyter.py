...
def draw_run_job(
    ...
    if uses_observables and uses_mitigation:
        name = "Observable Mitigation"
        G.add_edge(prev, name)
        prev = name
        merge_nodes.append(name)
        arch_edges.add((name, "Merge"))

    if uses_observables:
        name = "Observable Compute"
        G.add_edge(prev, name)
        prev = name
        merge_nodes.append(name)

    if not uses_observables and uses_mitigation:
        name = "Distribution Mitigation"
        G.add_edge(prev, name)
        prev = name
        merge_nodes.append(name)
        merge_nodes.append(name)
    ...
    if len(merge_nodes) > 1:
        ...
        G.add_edge("Merge", device)
    else:
        G.add_edge(prev, device)
    ...
    STYLES = {
        "layer": {"fill": "#d9e6f2", "stroke": "#2b2b2b"},
        "merge": {"fill": "#fff3cd", "stroke": "#d4a000"},
        "device": {"fill": "#e8f5e9", "stroke": "#2ca02c"},
    }
    ARROW_C = "#2b2b2b"
    TEXT_C = "#2b2b2b"
    ...
    fig, ax = plt.subplots(figsize=(max(5.0, SPACING * (n - 1) + 2.0), 2.4))
    ax.set_axis_off()
    ...
    for node, x in node_x.items():
        ...
        ax.text(
            x, 0, textwrap.fill(node, width=14), ha="center", va="center", fontsize=7.5, color=TEXT_C, zorder=4, linespacing=1.3
        )

    # Skip edges are routed above (positive y offset) to avoid crossing nodes
    seen: set = set()
    skip_row: dict = {}
    for u, v in G.edges():
        ...
        if (u, v) in seen:
            if is_arch:
                # duplicate of an arch edge → also draw the straight arrow
                ax.annotate(
                    "",
                    xy=(xv - NODE_W / 2, 0.0),
        ...
        if not is_arch:
            ...
            # Arch routed above the node row; each extra skip gets a higher lane
            lane = skip_row.get(gap, 0)
            skip_row[gap] = lane + 1
            y_off = NODE_H / 2 + 0.22 + lane * 0.28
            # elbow: source top-center → up → across → down → target top-center
            xs = [xu, xu, xv, xv]
            ys = [NODE_H / 2, y_off, y_off, NODE_H / 2]
            ax.plot(xs, ys, color=ARROW_C, lw=1.2, zorder=2)
            ax.annotate(
                "",
                xy=(xv, NODE_H / 2),
                xytext=(xv, y_off),
    ...
    if help:
        ...
        node_descriptions = {
            "Observable Mitigation": (
                "Mitigates user-provided circuits. May require additional circuits, "
                "producing two outputs that both feed into Merge."
            ),
            "Observable Compute": ("Groups observables and appends them as measurements to circuits."),
            "Distribution Mitigation": (
                "Mitigates the output probability distribution after execution. "
                "Passes both mitigated and raw results into Merge."
            ),
            "Merge": "Combines results from parallel processing branches.",
            "Packing": "Packs multiple circuits into a single one.",
        }
        ...
        for node in node_order:
            ...
            if desc is None:
                continue
            s = STYLES["merge"] if node == "Merge" else STYLES["layer"]
            handles.append(Patch(facecolor=s["fill"], edgecolor=s["stroke"], linewidth=1.2))
            labels.append(f"{node}: {desc}")

        if handles:
            # ~0.054 in/char at fontsize 6.5 + patch + margins
            ...
            leg = ax.legend(
                handles,
                labels,
                title="Each box is a processing unit in the data flow from user to device.",
                title_fontsize=6.5,
                fontsize=6.5,
                loc="upper left",
